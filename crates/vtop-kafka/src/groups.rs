...
impl Coordinator {
    ...
    pub fn heartbeat(
        ...
        match group.state {
            State::Stable => Ok(()),
            _ => Err(ErrorCode::RebalanceInProgress),
        }
    }
    ...
    /// Whether a commit under this membership is in order: a simple
    /// consumer's (generation -1, no member) always is; a member's must name
    /// its group's generation and find the group stable.
    pub fn check_commit(
        &self,
        group_id: &str,
        generation: i32,
        ...
        // Exactly -1 with no member (review): the protocol's one sentinel for
        // a commit outside managed membership; any other negative generation
        // is a generation the group never had.
        if generation == -1 && member_id.is_empty() {
            return Ok(());
        }
        let mut groups = self.lock();
        let group = groups
            .get_mut(group_id)
            .ok_or(ErrorCode::IllegalGeneration)?;
        let member = group
            .members
            .get_mut(member_id)
            .ok_or(ErrorCode::UnknownMemberId)?;
...
mod tests {
    ...
    /// An empty group id is refused before the simple-consumer bypass
    /// (review): nothing is stored under a group that names none.
    #[test]
    fn an_empty_group_id_never_commits() {
