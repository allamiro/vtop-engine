...
impl ApiKey {
    ...
    pub fn version_range(self) -> (i16, i16) {
        match self {
            ...
            // The group protocol, up to the last version before static
            // membership (#457 slice 2, review): JoinGroup v5, SyncGroup v3 and
            // Heartbeat v3 carry a `group.instance.id` this gateway does not
            // honour (KIP-345: instance-id mapping and fencing), so they are
            // not offered — a client configured for static membership is
            // refused at the header gate, as its own client library refuses
            // to start it below those versions, rather than admitted as the
            // dynamic member it is not. OffsetCommit starts at v5: v0 and v1 carry a
...
pub enum ErrorCode {
    ...
    /// A request the gateway cannot answer whole: an OffsetFetch naming no
    /// topics for a group that has committed more partitions than one answer
    /// carries — the client names the partitions it wants.
    InvalidRequest = 42,
}
