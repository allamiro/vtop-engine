...
pub fn decode_offset_commit(
    ...
    if version >= 7 {
        let _ = d.nullable_string("offsetCommit.groupInstanceId")?;
    }
...
pub fn consumer_assignment_partitions(bytes: &[u8]) -> Result<Vec<(String, i32)>, WireError> {
    ...
    for _ in 0..topic_count {
        ...
        let count = partitions_within(&mut d, "assignment.topics.partitions", &mut total)?;
        for _ in 0..count {
            out.push((name.clone(), d.i32("assignment.topics.partition")?));
        }
    }
