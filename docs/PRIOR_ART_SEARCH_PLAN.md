...
| **IBM Aspera FASP** | High-speed file transfer with integrity verification. | Integrity verification of transferred data. | Transport protocol, not telemetry batching/manifest archival; no source-offset commit model. | Low — rating retained on re-examination for the egress-transport milestone (#490): a proprietary transport with no public protocol specification is thinner prior art for a verified-bulk-transfer claim than the published protocols in the rows below, which cover the same ground with citable specifications. | Relevant only to "verify before considering transfer complete." |
| **UDT (UDP-based Data Transfer, IETF draft-gg-udt)** | Reliable rate-controlled bulk transfer over UDP with its own congestion control (DAIMD), built for high-bandwidth-delay-product links. | Bulk data over a datagram substrate on long fat links — the mechanism space the egress milestone's datagram path (#475) sits in. | A transport library, not an archival pipeline: no batching, no manifest, no verify-before-commit, no source-offset model; VTOP additionally does not ship its own congestion control (non-goal, stated in #475). | Low-medium on "rate-controlled reliable UDP bulk transfer" as background art. | Published specification and open implementation make this the citable representative of the accelerated-UDP-transfer family; closer art than any proprietary product in the same space. |
| **Tsunami UDP protocol** | Bulk file transfer using UDP data with TCP control, rate-based rather than window-based. | Datagram data path beside a reliable control path; explicit target-rate tuning of the kind #480's tuning surface names. | Research-grade file mover; no integrity manifest bound to a commit, no telemetry semantics, no verification gating anything. | Low. | Background art for the split control/data shape and rate-based sending; published papers and code. |
| **RBUDP (Reliable Blast UDP)** | Aggressive bulk UDP transfer with an aggregated-loss retransmission round after the blast. | Bulk-transfer-over-UDP mechanism family; bounded-rounds ARQ posture echoes why #475 rejects FEC (ARQ cost is bounded at a few rounds). | Assumes a provisioned/dedicated link and no congestion control at all; nothing about verification, manifests, or commit gating. | Low. | Useful to bound claims about datagram bulk transfer: the aggressive end of the design space, published with analysis. |
| **GridFTP** | FTP extensions for grid computing: parallel TCP streams, striped transfers across hosts, restart markers, per-transfer integrity options. | Parallel-stream bulk movement and transfer-restart machinery — the tuned-TCP end of the comparison #483's baseline gate formalizes. | A file-transfer service, not an archival commit engine: restart markers checkpoint the transfer, not a source offset gated on a verified stored manifest. | Low-medium on parallel-stream bulk transfer and restartable uploads. | The citable representative of "make TCP fast with parallelism" — the alternative the TCP-baseline gate (#483) measures before any datagram path is authorized. |
| **QUIC / HTTP-3 (RFC 9000, RFC 9114)** | Standardised encrypted multiplexed transport over UDP, with HTTP semantics mapped onto it. | The exact protocol stack #475 names for the datagram path: HTTP/3 over QUIC via an established library, not a bespoke protocol. | A transport standard, not a system: everything VTOP claims — verify-before-commit, bound manifests, source-offset gating — sits above it and is untouched by it. | Low as prior art against VTOP's claims; definitional for the datagram path itself. | Cite the RFCs wherever the datagram path is described; a standards-track stack is what keeps the amplification/anti-replay analysis out of this repository (per #475's non-goals). |
