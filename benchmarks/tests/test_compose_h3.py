...
Several things about this service are load-bearing and none is visible in any
...
The fourth is what the proxy can read. It runs as the owner of both 0600 keys
gen-h3-certs.sh mints, so file modes do not keep it out of the CA signing key —
only the mounts do. It gets the leaf certificate and the leaf key, as
individual files, and the lint refuses any mount that reaches ca-key.pem.
...
# Where gen-h3-certs.sh puts the material, relative to the compose file (which
# is what compose resolves a relative bind source against), and the one file in
# it the proxy must never be able to read.
TLS_DIR = "tls"
CA_SIGNING_KEY = posixpath.join(TLS_DIR, "ca-key.pem")


def _bind_mounts(service):
    """Every bind mount of a service as (host source, container target, spec).

    Both spellings, because the lint must not be dodged by switching syntax: the
    long form's `source`/`target`, and the short form's `src:dst[:mode]` for
    anything that looks like a path rather than a named volume. The source is
    normalized against the compose file's directory, so `./tls`, `tls/` and
    `./tls/../tls` all compare as the same place.
    """
    mounts = []
    for spec in service.get("volumes") or []:
        if isinstance(spec, dict):
            if spec.get("type") != "bind":
                continue
            source, target = str(spec.get("source", "")), str(spec.get("target", ""))
        else:
            parts = str(spec).split(":")
            if len(parts) < 2 or not parts[0].startswith((".", "/", "$", "~")):
                continue
            source, target = parts[0], parts[1]
        mounts.append((posixpath.normpath(source), posixpath.normpath(target), spec))
    return mounts


def _reaches(source, host_path):
    """Whether a bind of `source` exposes `host_path`: the file itself, or any
    directory above it. Both sides are relative to the compose file."""
    source = posixpath.normpath(source)
    host_path = posixpath.normpath(host_path)
    return source == "." or host_path == source or host_path.startswith(source + "/")


def test_the_reach_check_sees_a_directory_as_well_as_the_file_itself():
    # The lint below is only as good as this predicate, and a predicate that
    # compared file names alone would wave through the exact shape the review
    # found: `./tls` mounted whole, key and all.
    for exposing in ("./tls", "tls/", ".", "./", "./tls/ca-key.pem", "./tls/../tls"):
        assert _reaches(exposing, CA_SIGNING_KEY), (
            f"a bind of {exposing!r} exposes {CA_SIGNING_KEY}, and the reach "
            "check does not see it — the CA-key lint would pass the leak it exists for"
        )
    for innocent in ("./tls/h3-proxy.pem", "./tls/h3-proxy-key.pem", "./tlsx",
                     "./nginx-h3.conf.template"):
        assert not _reaches(innocent, CA_SIGNING_KEY), (
            f"a bind of {innocent!r} does not expose {CA_SIGNING_KEY}; a reach "
            "check that says it does would refuse the fix along with the leak"
        )


def test_the_ca_signing_key_is_not_reachable_from_the_h3_proxy(proxy):
    # The proxy runs as the owner of BOTH 0600 keys gen-h3-certs.sh mints, so
    # file modes do not separate them — only what is mounted does. ca-key.pem
    # signs the CA the h3 scenarios hand the engine through SSL_CERT_FILE; a
    # proxy that can read it can mint a certificate for any name the benchmark
    # client will believe, from inside the one lab service whose job is
    # accepting connections.
    leaking = [spec for source, _, spec in _bind_mounts(proxy)
               if _reaches(source, CA_SIGNING_KEY)]
    assert not leaking, (
        f"the proxy bind-mounts {leaking!r}, which exposes {CA_SIGNING_KEY} — the "
        "CA signing key — inside the container. Mount the leaf certificate and "
        "its key as individual files; nginx reads nothing else"
    )
...
def test_the_h3_proxy_refuses_to_start_without_its_tls_material(proxy):
    ...
    # syntax, which for a file mount would put an empty DIRECTORY where the
    # certificate belongs and fail deep inside nginx's TLS setup instead.
    mounts = [(source, spec) for source, _, spec in _bind_mounts(proxy)
              if _reaches(TLS_DIR, source)]
    assert mounts, (
        f"the proxy bind-mounts nothing from ./{TLS_DIR}, so nginx has no "
        "certificate to serve"
    )
    for source, spec in mounts:
        assert isinstance(spec, dict), (
            f"the TLS mount {spec!r} uses the short `src:dst` syntax, which "
            "creates a missing source as a directory and turns 'you never ran "
            "gen-h3-certs.sh' into an obscure TLS failure"
        )
        assert (spec.get("bind") or {}).get("create_host_path") is False, (
            f"the TLS mount {spec!r} must set bind.create_host_path: false, so "
            "`up` refuses instead of inventing an empty directory where "
            f"{source} belongs"
        )
        assert spec.get("read_only") is True, (
            f"the proxy only ever reads {source}; the mount must be read_only"
        )


def test_the_proxy_mounts_supply_every_certificate_path_the_template_reads(proxy, template):
    # The other half of narrowing the mount: a lint that only forbade the CA
    # key would pass a proxy that mounts nothing and cannot start. Every path
    # the template's TLS directives read must be the TARGET of a mount whose
    # SOURCE is the same-named file under ./tls — the name check is what stops
    # a swapped certificate and key from passing as "both mounted".
    paths = []
    for line in _directive_lines(template):
        found = re.match(
            r"^(ssl_certificate|ssl_certificate_key|ssl_trusted_certificate|"
            r"ssl_client_certificate)\s+(\S+?);", line)
        if found:
            paths.append(found.group(2))
    assert paths, (
        f"{os.path.basename(TEMPLATE_PATH)} names no ssl_certificate at all; "
        "this check would then assert nothing"
    )
    mounts = _bind_mounts(proxy)
    for path in paths:
        supplying = [source for source, target, _ in mounts
                     if target == posixpath.normpath(path)]
        assert supplying, (
            f"nginx reads {path}, and no bind mount targets it; the proxy would "
            f"die at startup. Mounts: {[spec for _, _, spec in mounts]!r}"
        )
        expected = posixpath.join(TLS_DIR, posixpath.basename(path))
        assert supplying == [expected], (
            f"{path} is supplied from {supplying!r}, not from {expected}; the "
            "file nginx loads under that name must be the one gen-h3-certs.sh "
            "minted under it"
        )
