...
def iperf_intervals(per_second, omit=0, ramp_mbps=None):
    """The report's own per-second stream, in the shape a real iperf3 emits one.
    ...
    for i in range(max(0, omit - 1)):
        stream.append({"sum": {"start": float(i), "end": i + 1.0, "seconds": 1.0,
                               "bytes": int(ramp * 1e6 / 8), "omitted": True}})
    at = 0.0
    for i, rate in enumerate(per_second):
        ...
        stream.append({"sum": {
            ...
            "start": 1.0 if straddles else at,
            "end": end,
            "seconds": 2.0 if straddles else 1.0,
            "bytes": int(rate * 1e6 / 8),
...
def iperf_report(mbps, seconds=30.0, sent_mbps=None, per_second=None, omit=0,
                 ramp_mbps=None, intervals=None):
    """An iperf3 --json report, in the shape the real one has.
    ...
    `per_second` hands a test an UNEVEN flow — one rate per second of the window
    — which is what makes "the rate over the covered span" and "the window's
    average" two different numbers rather than the same one twice.
    """
    sent = mbps * 1.2 if sent_mbps is None else sent_mbps
    if intervals is None:
        intervals = iperf_intervals(per_second or [sent] * max(1, int(round(seconds))),
                                    omit=omit, ramp_mbps=ramp_mbps)
    return json.dumps({
        "intervals": intervals,
        "end": {
...
class ScriptedCompetitor:
    ...
    def __init__(self, rates, seconds=30.0, refuse=None, delay=0.0, output=None, during=None,
                 contended_report=None, runs_for=None):
        ...
        # HOW LONG THE CONTENDED COMMAND TAKES IN WALL CLOCK, whatever its
        # report claims it measured. The module places the measured window
        # between the instant the command was launched and the instant its
        # report came back, so a test about where that window lands has to spend
        # the seconds a real `docker exec` spends: the difference between the
        # two IS the overhead the derivation takes off the attributable span.
        # Held to a deadline rather than slept for, so the cycles a test runs
        # inside the window come out of this budget instead of extending it.
        #
        # The middle phase only, like `contended_report` and for the same
        # reason: the solo windows are never placed on the run's clock, so
        # charging the suite three times over would buy nothing.
    ...
    def __call__(self, argv):
        ...
        # thread and given the 1-based phase number. A test that places an
        # engine cycle from its own thread is racing the flow's closing
        # boundary sample, which the flow takes the instant this call returns —
        # and the only way to win that race from outside is a sleep long enough
        # to absorb a scheduling hiccup, which every run then pays for. Placed
        ...
        if self.runs_for and len(self.calls) == 2:
            time.sleep(max(0.0, entered + self.runs_for - time.monotonic()))
        if self.refuse is not None:
            return self.refuse
        if self.output is not None:
            return 0, self.output
        if self.contended_report is not None and len(self.calls) == 2:
            self.rates.pop(0)
            return 0, self.contended_report
        return 0, iperf_report(self.rates.pop(0), self.seconds)
...
def spec(seconds=30, omit=0):
    # omit=0 by default so a test's scripted competitor, which returns
    # instantly, is not waiting out a real ramp. The ramp's own effect on the
    # window boundary has its own test, which sets it.
...
def test_the_command_carries_the_window_the_ramp_and_the_json_report():
    ...
    assert "-J" in argv, "the report is parsed, not scraped from the human-facing output"
...
def test_a_report_with_no_receiver_summary_is_refused_rather_than_falling_back():
    ...
    only_sent = json.dumps({"end": {"sum_sent": {"bits_per_second": 9e6, "seconds": 30.0,
                                                 "bytes": 1}}})
    ...
    with pytest.raises(ShapingError, match="not an iperf3 JSON report"):
    ...
    with pytest.raises(ShapingError, match="unable to connect to server"):
        parse_report(json.dumps({"error": "unable to connect to server"}), spec(30), "alone after")
...
class ScriptedEngine:
    ...
    def cycle(self, handle, started_at, committed):
        time.sleep(SCRIPTED_CYCLE_SECONDS)
        self.total += committed
        handle.note_progress(started_at)


def earliest_opening(handle):
    """The earliest instant the contended window can have opened.

    The module derives the window from the report AFTER the flow returns, so
    there is no opening boundary to read while the flow is running — and a test
    that placed cycles against one it invented would be asserting about its own
    arithmetic. What a test CAN hold on to is the near bound the derivation
    itself uses: iperf3 cannot have begun measuring before its command was
    launched, plus the ramp it omits.

    On a scripted flow whose report claims a window longer than the command
    takes — every test here that does not set `runs_for` — that bound is also
    where the window ends up, which is what makes a cycle placed on it land
    inside. The tests that DO set `runs_for` are the ones about the gap between
    the two, and they place their cycles at offsets from this bound knowing the
    window opens later.
    """
    wait_for(lambda: handle._launched_at is not None, "launched its command")
    return handle._launched_at + handle.spec.omit


def cycles_inside_the_window(handle_box, engine, placements):
    ...
    second before it — the straddle whose bytes cannot be split. Every cycle
    ...
    The hook WAITS for that boundary before running anything: with a non-zero
    omit the window cannot open until the ramp has elapsed, and a cycle handed a
    start in the future would be recorded as having returned before it began.
    """
    ...
    def during(phase):
        if phase != 2:  # the contended window is the second of the three
            return
        handle = handle_box[0]
        opened_at = earliest_opening(handle)
        wait_until(opened_at, "reached the earliest instant its window can have opened")
        for before, committed in placements:
            engine.cycle(handle, opened_at - before, committed)
...
def run_three_phases(rates, vtop_bytes_after=1_000_000, **kwargs):
    ...
    box = ScriptedCompetitor(
        rates,
        during=cycles_inside_the_window(handle_box, engine, [(0, vtop_bytes_after)]),
        **kwargs)

    with contended(spec(30), vtop_bytes=engine, run=box, log=lambda _m: None) as handle:
...
def test_a_contended_window_that_outlived_the_engines_loop_is_refused():
    ...
    with pytest.raises(ShapingError, match="engine's window closed") as exc:
        with contended(spec(30), vtop_bytes=lambda: 0, run=box, log=lambda _m: None) as handle:
            handle.start_with_vtop()
...
def test_a_flow_that_failed_reports_its_own_failure_and_not_the_scheduling_one():
    ...
    def refused_after_a_moment(argv):
        if not hasattr(refused_after_a_moment, "seen"):
            ...
            return 0, iperf_report(9.0)
        time.sleep(0.5)
        return 1, "iperf3: error - the server is busy running a test"

    with pytest.raises(ShapingError, match="the server is busy") as exc:
        with contended(spec(30), vtop_bytes=lambda: 0, run=refused_after_a_moment,
...
def test_a_contended_window_that_was_never_opened_is_refused():
    ...
    with pytest.raises(ShapingError, match="never opened"):
        with contended(spec(30), vtop_bytes=lambda: 0, run=box,
                       log=lambda _m: None) as handle:
            handle.close()
...
def test_a_competitor_that_fails_in_the_contended_window_fails_the_run():
    ...
    def flaky(argv):
        ...
        if len(calls) == 1:
            return 0, iperf_report(9.0)
        return 1, "iperf3: error - control socket has closed unexpectedly"

    with pytest.raises(ShapingError, match="control socket has closed"):
        with contended(spec(30), vtop_bytes=lambda: 0, run=flaky, log=lambda _m: None) as handle:
            handle.start_with_vtop()
            handle.close()
...
def test_vtops_window_opens_after_iperfs_omitted_ramp():
    ...
    box = ScriptedCompetitor(
        ...
        during=cycles_inside_the_window(handle_box, engine,
                                        [(0.5, 7_000_000), (0, 5_000_000)]))

    started = time.monotonic()
    with contended(CompetitorSpec(mode="bulk", seconds=30, omit=1),
...
def test_a_window_that_saw_no_engine_cycle_is_refused_rather_than_recorded():
    ...
    handle_box: list = []
    box = ScriptedCompetitor(
        [9.0, 4.0, 9.0], during=cycles_inside_the_window(handle_box, engine, [(0, 0)]))
    with contended(spec(30), vtop_bytes=engine, run=box, log=lambda _m: None) as handle:
        handle_box.append(handle)
        handle.start_with_vtop()
        wait_for(lambda: handle._collected is not None, "collected its report")
...
def test_a_cycle_that_returned_after_the_window_closed_is_charged_to_neither_side():
    ...
    box = ScriptedCompetitor(
        [9.0, 4.0, 9.0],
        during=cycles_inside_the_window(handle_box, engine, [(0, 1_000_000)]))
    with contended(spec(30), vtop_bytes=engine, run=box, log=lambda _m: None) as handle:
        handle_box.append(handle)
        handle.start_with_vtop()
        wait_for(lambda: handle._collected is not None, "collected its report")
        ...
        engine.cycle(handle, earliest_opening(handle), 7_000_000)
        handle.close()
...
def test_a_window_of_only_straddling_cycles_is_refused_even_though_progress_landed_inside_it():
    ...
    box = ScriptedCompetitor(
        [9.0, 4.0, 9.0],
        during=cycles_inside_the_window(handle_box, engine, [(1.0, 7_000_000)]))
    with contended(spec(30), vtop_bytes=engine, run=box, log=lambda _m: None) as handle:
        handle_box.append(handle)
        handle.start_with_vtop()
        wait_for(lambda: handle._collected is not None, "collected its report")
        engine.cycle(handle, earliest_opening(handle), 3_000_000)
        with pytest.raises(ShapingError) as exc:
            handle.close()
...
def cycles_at(handle_box, engine, placements):
    """A `during` hook that runs engine cycles at real offsets into the flow.

    `placements` are (seconds after `earliest_opening`, bytes) pairs, and the
    wait for each one is a real deadline poll rather than a start supplied to
    `note_progress`. `cycles_inside_the_window` constructs its starts, which is
    right for tests about which side of an EDGE a cycle falls on; these tests
    are about where in the flow's own run — and so in its interval stream — a
    cycle lands, so the cycle has to occupy a span the clock agrees with.

    The offsets are from the flow's LAUNCH plus its ramp, not from the window
    the module derives: with a `runs_for` flow the window opens later than that
    by the overhead, and it is exactly that difference these tests measure.
    """
    ...
    def during(phase):
        ...
        for after, committed in placements:
            wait_until(opened_at + after, f"reached {after}s past its opening boundary")
            engine.cycle(handle, time.monotonic(), committed)

    return during


def run_with_report(report, vtop_bytes=2_400, seconds=30.0, window=30, placements=None,
                    runs_for=None):
    ...
    engine = ScriptedEngine()
    handle_box: list = []
    during = (cycles_at(handle_box, engine, placements) if placements
              else cycles_inside_the_window(handle_box, engine, [(0, vtop_bytes)]))
    box = ScriptedCompetitor([9.0, 8.0, 9.0], seconds=seconds, during=during,
                             contended_report=report, runs_for=runs_for)
    with contended(CompetitorSpec(mode="bulk", seconds=window, omit=0),
                   vtop_bytes=engine, run=box, log=lambda _m: None) as handle:
        handle_box.append(handle)
        handle.start_with_vtop()
        wait_for(lambda: handle._collected is not None, "collected its report")
...
def test_the_index_is_taken_over_the_span_vtop_was_measured_over_not_the_windows_average():
    ...
    handle = run_with_report(
        iperf_report(8.0, seconds=30.0, sent_mbps=8.5, per_second=UNEVEN_PER_SECOND))
...
def test_a_flow_whose_rate_changed_is_not_represented_by_its_average_over_the_window():
    ...
    flat = run_with_report(iperf_report(8.0, seconds=30.0, sent_mbps=8.5))
...
def test_a_report_with_no_interval_stream_is_refused_rather_than_averaged():
    ...
    stripped = json.loads(iperf_report(4.0, seconds=30.0))
    stripped.pop("intervals")
    with pytest.raises(ShapingError, match="no interval stream") as exc:
        parse_report(json.dumps(stripped), spec(30), "with VTOP")
...
def test_a_span_the_report_cannot_speak_for_is_refused_rather_than_filled_in():
    ...
    with_a_hole = iperf_report(4.0, seconds=6.0, intervals=[
        {"sum": {"start": 0.0, "end": 0.1, "seconds": 0.1, "bytes": 100_000,
                 "omitted": False}},
    ...
    with pytest.raises(ShapingError, match="no interval of the competing flow's report") as exc:
        run_with_report(with_a_hole, seconds=6.0, window=6,
                        placements=[(0.3, 1_000_000)])
    message = str(exc.value)
    assert "0-6.000s" in message and "after the window opened" in message, (
...
def test_a_stream_that_only_touches_the_span_at_both_ends_does_not_cover_it():
    ...
    # The same report is still usable where it DOES cover the span: the rule is
    # about coverage, not about condemning a report with a hole anywhere in it.
    inside_a_kept_second, touched = competitor.competitor_bytes_over(
        holed, 1_000.0, 1_000.2, 1_000.8)
    assert touched == 1 and inside_a_kept_second > 0, (
        "a span that lies wholly inside one surviving interval is measured, not refused — "
        "the stream describes those seconds perfectly well")
...
# exit at the other — and the run cannot see how it split. These two numbers
# are what put the derived window at 0.4-0.8s past the launch instead of
# 0-0.8s, and they are held to a DEADLINE by the scripted flow rather than
# slept for, so the cycles a test places inside the window come out of the
# command's own budget instead of moving its far edge.
OVERHEADED_WALL_SECONDS = 1.2
...
def test_a_cycle_that_ran_during_the_flows_setup_is_charged_to_neither_side():
    ...
    # setup took that afternoon, and on a slow enough one before the flow had
    # started at all. Cycles from the setup were then charged to a window that
    # had not opened and paired with the competitor's post-ramp seconds.
    #
    # Nothing sleeps now: the window's length is the report's, and its position
    # is pinned between the launch and the collection. A cycle 0.2s in ran while
    # the command may still have been connecting, so no admissible placement of
    # the window contains it; one at 0.6s ran inside every placement of it.
    handle = run_with_report(
        iperf_report(4.0, seconds=OVERHEADED_MEASURED_SECONDS),
        seconds=OVERHEADED_MEASURED_SECONDS, window=0,
        runs_for=OVERHEADED_WALL_SECONDS,
        placements=[(0.2, 7_000_000), (0.6, 1_000_000)])

    assert handle.vtop_attributed_cycles == 1, (
        "a cycle that ran before the earliest instant the report's own window can have "
        "opened is charged to neither side: the competitor was not measuring yet, or may "
        "not have been, and a share paired with seconds the flow never reported is the "
        "corruption the derived boundary exists to prevent")
    assert handle.vtop_bytes_in_window == 1_000_000, (
    ...
    assert handle.window_unaccounted_seconds == pytest.approx(
        OVERHEADED_WALL_SECONDS - OVERHEADED_MEASURED_SECONDS, abs=0.1), (
        "and the run RECORDS how loosely it could pin the window rather than assuming the "
        "overhead away: this is the number a reader needs to know how much of the window "
        "was spent proving where it was")
    assert handle.vtop_bytes_across_window == 1_000_000, (
        "the counter's movement across the window excludes the setup cycle too: those 7 MB "
        "were already in the total when the window opened, and the whole point of this "
        "figure is to be the counter's own honest view of the SAME two edges the "
        "attributed bytes were taken between")


def test_a_cycle_that_returned_in_the_collection_tail_is_charged_to_neither_side():
    # The far edge of the same arithmetic. A report is collected at iperf3's
    # last measured second PLUS the time the client took to exchange results,
    # exit and travel back through docker exec, and the report says nothing
    # about that tail. A cycle returning inside it would stretch the shared span
    # past the last second either flow was measured on — and since the
    # surrounding seconds still overlap, that stretches the denominator without
    # adding bytes and silently understates the competitor.
    handle = run_with_report(
        iperf_report(4.0, seconds=OVERHEADED_MEASURED_SECONDS),
        seconds=OVERHEADED_MEASURED_SECONDS, window=0,
        runs_for=OVERHEADED_WALL_SECONDS,
    ...
    assert handle.vtop_attributed_cycles == 1, (
        "only the cycle that ran while the competitor must still have been measuring may "
        "be charged to the shared span; the one that returned once the window's latest "
        "possible close had passed is charged to neither side, exactly like a cycle that "
        "crossed an edge")
    assert handle.vtop_bytes_in_window == 1_000_000, (
