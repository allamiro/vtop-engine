...
@pytest.mark.parametrize("filed", [
    f'VTOP_H3_PORT="{MOVED_PORT}"',
    f"VTOP_H3_PORT='{MOVED_PORT}'",
    f'VTOP_H3_PORT="{MOVED_PORT}" # moved off a busy port',
    f"VTOP_H3_PORT={MOVED_PORT} # moved off a busy port",
])
def test_a_quoted_or_commented_port_resolves_identically_in_both_languages(
        monkeypatch, env_file, filed):
    # `VTOP_H3_PORT="9543"` is valid dotenv: Compose strips the quotes and
    # publishes 9543, and the harness's parser agrees. The shell used to return
    # the quotes, so verify-h3.sh built a URL around a port with quotes in it and
    # failed a correctly configured proxy (review).
    env_file.write_text(filed + "\n", encoding="utf-8")
    monkeypatch.setattr(engine, "_ENV_FILE", str(env_file))
    assert resolved_by_the_shell(env_file) == str(MOVED_PORT), (
        f"the shell resolved {resolved_by_the_shell(env_file)!r} from {filed!r}; compose "
        f"publishes {MOVED_PORT}, so verify-h3.sh would probe a port nothing listens on"
    )
    assert str(engine.h3_proxy_host_port()) == resolved_by_the_shell(env_file), (
        "and the harness must resolve the same port from the same line"
    )
