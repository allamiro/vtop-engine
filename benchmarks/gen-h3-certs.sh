...
#
# Usage: gen-h3-certs.sh [output-dir]     (default: benchmarks/tls)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CERT_DIR="${1:-$SCRIPT_DIR/tls}"
...
if [[ ! -f ca.pem || ! -f "$LEAF.pem" || ! -f "$LEAF-key.pem" ]]; then
  needs_mint=1
  reason="material missing"
elif ! openssl x509 -in ca.pem -checkend "$RENEW_WITHIN_SECONDS" -noout >/dev/null 2>&1; then
  needs_mint=1
  reason="CA expired or expiring within two days"
elif ! openssl x509 -in "$LEAF.pem" -checkend "$RENEW_WITHIN_SECONDS" -noout >/dev/null 2>&1; then
  needs_mint=1
...
# proxy dies at startup, having never read the key this script just minted for
# it, and the failure names a cache directory rather than this file.
#
...
reached the certificate this run produced.

Nothing was written to $SCRIPT_DIR/.env: VTOP_BENCH_UID=0 is an identity that cannot work,
and filing it would move the failure to a place that does not name it. Mint the material as
the unprivileged user that will run the lab —
  sudo -u <user> $SCRIPT_DIR/$(basename "${BASH_SOURCE[0]}")
— or hand this material over to them and re-run:
  chown <uid>:<gid> $PWD/ca-key.pem $PWD/$LEAF-key.pem
EOF
  exit 1
fi
