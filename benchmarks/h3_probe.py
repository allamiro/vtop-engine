...
def _authority(host: str, port: int) -> str:
    """The `:authority` a request to `host`:`port` names, in URI syntax.

    `urlsplit(...).hostname` hands back an IPv6 literal with its brackets
    stripped, and `::1:9443` is not an authority at all — it is an address
    whose last group happens to be 9443 (review). RFC 3986 puts a literal in
    brackets precisely so the port stays separable, so they go back on here.
    Only here: SNI and certificate matching take the bare address, and
    `getaddrinfo` does not understand brackets either, so `host` itself stays
    as `urlsplit` returned it.
    """
    if ":" in host:
        return f"[{host}]:{port}"
    return f"{host}:{port}"
